...
export default async function AdminColoringPages() {
  const pages = await db.select({ id: coloringPages.id, title: coloringPages.title, slug: coloringPages.slug, isPublished: coloringPages.isPublished, featured: coloringPages.featured, categoryName: categories.name }).from(coloringPages).leftJoin(categories, (fields) => fields).orderBy(desc(coloringPages.publishedAt));
