    ...
    <main className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
              View site
            </Link>
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="grid gap-5 md:grid-cols-3">
          {stats.map((stat) => (
            <Link key={stat.label} href={stat.href} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        <section className="mt-10 rounded-2xl border border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mt-1 text-sm leading-6 text-amber-900">
            Configure ADMIN_USERNAME, ADMIN_PASSWORD and ADMIN_SESSION_SECRET in your deployment environment before using the panel in production.
          </p>
        </section>
      </div>
    </main>
